...
unsigned char subtask3(void)
{
	...
	//insert application	
	send_UARTString(">>>>subtask3 , Flag set to 0\r\n");
	set_flag(flag_TASK_SWAP,Disable);
			
	_TaskEnd_
}

unsigned char task_CallSubTask(void)
{
//	static int i;  
	...
	while(1)
	{
		...
		//insert application		
		if (is_flag_set(flag_TASK_SWAP))
		{
			send_UARTString(">>>>CallSubTask\r\n");
			CallSub(subtask3); 
		}
	}
	_TaskEnd_
...
unsigned char task_Flag_Swap(void)
{
	static int i;  
	...
	while(1)
	{
		...
		set_flag(flag_TASK_SWAP,Enable);

		send_UARTString(">>>>Flag_Swap(2000ms) , Flag set to 1\r\n");	
	}
	_TaskEnd_
}

unsigned char task_logger_5000ms(void)
{
	static int i;  
...
unsigned char task_logger_250ms(void)
{
//	static int i;  
